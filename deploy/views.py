        ...
        if request.method == 'GET':
            if request.GET.has_key('delete'):
                ...
                try:
                    ...
                    with open('./saltconfig/nodegroup.conf', 'w') as f:
                ...
                if request.is_ajax():
                    ...
                    for i in range(0, int(targets['level']) + 1):
                        ...
                        if check_type == 'panel-group':
                            ...
                            expr_form = 'list'
                        try:
                            jid = sapi.remote_execution(tgt_selects, 'cmd.run', args, expr_form)
                            rst_source = sapi.salt_runner(jid)
                            rst = rst_source['info'][0]['Result']
                            result.append(rst)
        ...
        if request.method == 'GET':
            if request.user.has_perms(['deploy.view_filemanage', 'deploy.edit_filedownload']):
                if request.GET.get('type') == 'download':
                    ...
                    ret_bak = sapi.file_bak(tgt_select, 'cp.push', remote_file, expr_form)
                    if tgt_select == 'localhost':
                        return render(request, 'redirect.html', {})
                    remote_path = os.path.dirname(remote_file)
                    fname = os.path.basename(remote_file)

                    dl_path = 'media/salt/filedownload/user_%s/%s' % (request.user.id, remote_path.replace('/', '_'))
                    if not os.path.exists(dl_path):
                        os.makedirs(dl_path)
                    random_path = './media/salt/.cache/{}'.format(get_token(12))
...
def salt_ajax_file_upload(request):
    ...
    if request.is_ajax():
        ...
        src_dir = 'fileupload/user_%s' % (request.user.id)
